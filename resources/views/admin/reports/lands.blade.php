...
<div class="table-wrap">
    <table class="report-table">
        ...
        <tr>
            ...
            <th style="width: 260px;">عنوان الارض بالتفصيل</th>
            <th style="width: 160px;">المساحه</th>
            <th style="width: 160px;">سعر المتر</th>
            <th style="width: 160px;">اجمالي السعر</th>
            <th style="width: 200px;">اسم المالك / الوسيط</th>
            <th style="width: 200px;">رقم هاتف المالك / الوسيط</th>
        ...
        @foreach($lands as $land)
            <tr data-id="{{ $land->id}}" style="cursor:pointer;">
                <td class="c-id" onclick="window.location='{{ route('lands.show', $land->id) }}'">{{$land->user?->name}}</td>
                <td class="c-id" onclick="window.location='{{ route('lands.show', $land->id) }}'">{{$land->address}}</td>
                <td class="c-id" onclick="window.location='{{ route('lands.show', $land->id) }}'">{{$land->address_details}}</td>
                ...
                <td class="c-advertiser-type" onclick="window.location='{{ route('lands.show', $land->id) }}'">{{$land->seller_name}}</td>
                <td class="c-advertiser-type" onclick="window.location='{{ route('lands.show', $land->id) }}'">{{$land->seller_phone_number}}</td>
                ...
                <td class="c-real-state-address" onclick="window.location='{{ route('lands.show', $land->id) }}'">{{$land->land_date_register}}</td>
            </tr>
        @endforeach
        </tbody>
    </table>
        ...
        <div class="modal-content search-modal">
            <div class="modal-body p-0">
                <div class="panel-modal">
                    ...
                    <form action="{{route('admin.reports.lands')}}" method="GET" autocomplete="off">
...
<script>
    ...
    document.getElementById("searchBtn").addEventListener("click", () => {
        const searchForm = document.querySelector('#searchModal form[action="{{ route('admin.reports.lands') }}"]');
...
<script>
    ...
    document
        .querySelectorAll('form[action="{{ route('admin.reports.lands') }}"]')
        .forEach((form) => {
