...
<div class="app">
    ...
    <div class="table-wrap">
        <table class="props" id="propsTable">
            ...
            <tr>
                ...
                <th style="width: 50px;">كود المعاينه</th>
                <th style="width: 50px;">القسم</th>
                <th style="width: 100px;">تاريخ المعاينه</th>
            ...
            @foreach($clients as $client)
             <tr data-id="{{ $client->id}}" style="cursor:pointer;">
                <td class="c-real-state-address">{{$client->name}}</td>
                <td class="c-real-state-price">{{$client->phone}}</td>
                <td class="c-real-state-type-label">{{$client->code}}</td>
                <td class="c-real-state-type-label">{{$client->departmentLabel}}</td>
                <td class="c-real-state-type-label">
                    @if($client->inspection_date != null)
                        <span class="badge bg-success">  {{$client->inspection_date}}</span>
                    @else
                        <span class="badge bg-danger">لا يوجد اي تاريخ معاينه حاليا!</span>
                    @endif
                </td>
                <td class="c-real-state-type-label" onclick="window.location='{{ route('clients.show', $client->id) }}'">

                    @if($client->status == 'waiting')
                        <span class="badge bg-secondary">انتظار</span>
                    @elseif($client->status == 'inspection')
                        <span class="badge bg-info">معاينة</span>
                    @elseif($client->status == 'inspection_accepted')
                        <span class="badge bg-success">معاينة مقبولة</span>
                    @else
                        <span class="badge bg-danger">معاينة مرفوضة</span>
                    @endif

                </td>
                <td class="c-real-state-type-label">{{$client->user?->name}}</td>
                <td class="t-action">
                    <a style="color: #3f5564" href="{{route('admin.clients.edit',$client->id)}}" class="js-edit"><i class="fa-solid fa-pen-to-square"></i></a>
                </td>
                <td class="t-action">
    ...
    <div class="panel">
        ...
        <form action="{{route('clients.create')}}" method="POST" enctype="multipart/form-data" autocomplete="off">
            @csrf
            @method('POST')
            ...
            <div class="mb-2">
                <label class="form-label">كود المعاينه (اختياري)</label>
                <input name="code"
            ...
            <div class="mb-2">
                ...
                <select name="department" class="form-select @error('department') is-invalid @enderror">
                    ...
                    <option value="state_sale" >عقار بيع</option>
                    <option value="state_rent">عقار ايجار</option>
                    <option value="land_sale">ارض بيع</option>
            ...
            <div class="mb-2">
                ...
                <select name="client_type" class="form-select @error('client_type') is-invalid @enderror">
                    <option disabled>نوع العميل</option>
                    <option value="client" >صاحب عقار</option>
                    <option value="company">شركه عقارات</option>
            ...
            <div class="mb-2">
                <label class="form-label">تاريخ المعاينه (اختياري)</label>
            ...
            <div class="mb-2">
                <label class="form-label"> حاله المعاينه (اختياري)</label>
                <select name="status" class="form-select @error('status') is-invalid @enderror">
                    <option disabled>حاله المعاينه</option>
                    <option value="waiting" >انتظار</option>
                    <option value="inspection">معاينه</option>
                    <option value="inspection_accepted">معاينه مقبوله</option>
                    <option value="inspection_refused">معاينه مرفوضه</option>
            ...
            <div class="modal-body p-0">
                <div class="panel-modal">
                    <div class="panel-head">
                        ...
                        <p class="panel-title">بحث العملاء</p>
                    </div>
            ...
            <div class="modal-body p-0">
                ...
                <div id="editModalLoader" class="edit-loader">
                    <div class="loader-box">
                        ...
                        <div class="loader-text">جاري تحميل بيانات العميل...</div>
                    </div>
                </div>
                <!-- Content -->
...
<script>
    ...
    document.getElementById("openAddModalBtn").addEventListener("click", () => {
        const addForm = document.querySelector('#addModal form[action="{{ route('clients.create') }}"]');
...
<script>
    ...
    document.getElementById("searchBtn").addEventListener("click", () => {
        const searchForm = document.querySelector('#searchModal form[action="{{ route('clients.index') }}"]');
...
<script>
    ...
    const deleteUrlTemplate = `{{ route('clients.delete', ':id') }}`;
...
<script>
    ...
    document
        .querySelectorAll('form[action="{{ route('clients.index') }}"]')
        .forEach((form) => {
