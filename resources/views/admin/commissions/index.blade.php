...
<div class="app-topbar">
    ...
    <div class="title">عموله الموظفين</div>

    <button class="icon-btn" type="button" title="بحث" id="searchBtn">
        <i class="fa-solid fa-magnifying-glass"></i>
    </button>

    <button class="icon-btn" type="button" title="تحديث" onclick="window.location.href='{{ route('employee.commissions.index') }}'">
...
<div class="app">
    ...
    <div class="table-wrap">
        <table class="props" id="propsTable">
            ...
            @foreach($commissions as $commission)
             <tr data-id="{{ $commission->id}}" style="cursor:pointer;">
                <td class="c-real-state-address">{{$commission->employee?->name}}</td>
                <td class="c-real-state-address">{{$commission->total_money}}</td>
                <td class="c-real-state-address">{{$commission->description}}</td>
                <td class="c-real-state-address">{{$commission->transaction_date}}</td>

                <td class="t-action">
                    <a style="color: #3f5564" href="{{route('employee.commissions.edit',$commission->id)}}" class="js-edit"><i class="fa-solid fa-file-pen"></i></a>
                </td>
                <td class="t-action">
        ...
        <div class="card-footer">
            <div>{{ $commissions->appends(request()->query())->links() }}</div>
        </div>
    </div>
    ...
    <div class="panel">
        ...
        <form action="{{route('employee.commissions.create')}}" method="POST" enctype="multipart/form-data" autocomplete="off">
            @csrf
            @method('POST')
            ...
            <div class="mb-2">
                ...
                <select name="description" class="form-select">
                    <option value="" selected disabled>اختر الوصف</option>
                        <option value="عموله عقد ايجار">عموله عقد ايجار</option>
                        <option value="عموله عقد بيع">عموله عقد بيع</option>
            ...
            <div class="modal-body p-0">
                <div class="panel-modal">
                    <div class="panel-head">
                        ...
                        <p class="panel-title">بحث عموله الموظفين</p>
                    </div>


                    <form action="{{ route('employee.commissions.index') }}" method="GET" autocomplete="off">
            ...
            <div class="modal-body p-0">
                ...
                <div id="editModalLoader" class="edit-loader">
                    <div class="loader-box">
                        ...
                        <div class="loader-text">جاري تحميل بيانات عموله الموظف...</div>
                    </div>
                </div>
                <!-- Content -->
        ...
        <div class="modal-content">
            <div class="modal-body p-4">
                ...
                <p class="mb-3" style="color:#64748b;font-size:13px;">
                    هل أنت متأكد أنك تريد حذف هذه العموله؟ لا يمكن التراجع عن هذا الإجراء.
                </p>
...
@if(session('commission_index_error'))
    <div id="loginSuccessBar" class="login-success-bar show">
        {{ session('commission_index_error') }}
    </div>
@endif
<!-- Bootstrap JS -->
...
<script>
    ...
    document.getElementById("openAddModalBtn").addEventListener("click", () => {
        const addForm = document.querySelector('#addModal form[action="{{ route('employee.commissions.create') }}"]');
...
<script>
    ...
    document.getElementById("searchBtn").addEventListener("click", () => {
        const searchForm = document.querySelector('#searchModal form[action="{{ route('employee.commissions.index') }}"]');
...
<script>
    ...
    const deleteUrlTemplate = `{{ route('employee.commissions.delete', ':id') }}`;
...
<script>
    ...
    document
        .querySelectorAll('form[action="{{ route('employee.commissions.index') }}"]')
        .forEach((form) => {
